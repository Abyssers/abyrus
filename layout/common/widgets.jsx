...
function getColumnCount(widgets, config, page) {
    return (
        [hasColumn(widgets, "left", config, page), hasColumn(widgets, "right", config, page)].filter(v => !!v).length +
        1
    );
}
...
function isColumnSticky(config, position) {
    return typeof config.sidebar === "object" && position in config.sidebar && config.sidebar[position].sticky === true;
}
    ...
    render() {
        ...
        return (
            <div
                ...
                {position === "left" && hasColumn(config.widgets, "right", config, page) ? (
                    <div
                        class={classname({
